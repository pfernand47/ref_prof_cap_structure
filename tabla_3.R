# ---------------------------------
# Table 3: Different (symmetric) thresholds for the rebalancing dummy
# ---------------------------------
thresholds=seq(0.09,0.01,by=-0.01)
...
cnames = as.character(thresholds)
finalcnames=c()
...
for(r in thresholds){
  ...
  d$dum = dum
  d$inter = inter
  p = pdata.frame(d,index = c('gvkey','time'),row.names=FALSE) # this creates a 'panel data frame' with firm and time indices
  ...
  m["debt.th",r] = paste("$T_d=",r,"$",sep="")
  m["equity.th",r] = paste("$T_e=",r,"$",sep="")
  m["Controls",r] = "yes"
...
cap=c("\\centerline{\\textbf{Regression Results: Symmetric Refinancing Firm Definition}}")
lab=c("tab:symmetric")
table=xtable(m,caption=cap,label=lab)
align(table)=c("r",rep("c",ncol(m)))
path = paste(getwd(),"/Results/tabla_3.tex",sep='')
